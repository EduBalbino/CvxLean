...
lemma diag_inv_mul_diag_eq_one_of_upperTriangular [Fintype m] [LinearOrder m]
...
lemma diag_inv_mul_diag_eq_one_of_lowerTriangular [Fintype m] [LinearOrder m]
    [Invertible M] (hM : lowerTriangular M) (k : m) : M⁻¹ k k * M k k = 1 := by
  letI : Unique {a // OrderDual.toDual a = k} :=
    ⟨⟨⟨k, rfl⟩⟩, fun j => Subtype.ext j.property⟩
  have h := congr_fun (congr_fun
    (toSquareBlock_inv_mul_toSquareBlock_eq_one hM k) ⟨k, rfl⟩) ⟨k, rfl⟩
